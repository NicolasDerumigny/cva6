...
int main() {
...
#ifndef PLAT_AGILEX
  init_uart(CLOCK_FREQUENCY, UART_BITRATE); // not needed in intel setup as UART
                                            // IP is already configured via HW
#endif
  ...
  if (ret) {
    ...
    print_uart(" booting!\r\n");
#ifndef PLAT_AGILEX
    res = gpt_find_boot_partition(
        (uint8_t *)0x80000000UL,
        2 * 16384); // linux boot not yet supported for altera
    if (res != 0) {
      while (1)
        ;
    }
...
#ifdef PLAT_AGILEX // linux boot not yet supported for altera
  while (1) {
    // do nothing
  }
#endif

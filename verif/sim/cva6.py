...
from cva6_compare_text_traces import *
    ...
    for entry in yaml_data:
        if entry["tool"] == simulator:
            ...
            for i in range(len(compile_cmd)):
                if ("cov_opts" in compile_spec) and cov:
                    compile_cmd[i] = re.sub(
                        "<cov_opts>", compile_spec["cov_opts"].rstrip(), compile_cmd[i]
                    )
                else:
            ...
            if ("cov_opts" in entry["sim"]) and cov:
                sim_cmd = re.sub(
                    "<cov_opts>", entry["sim"]["cov_opts"].rstrip(), sim_cmd
                )
            else:
            ...
            if "env_var" in entry:
                for env_var in entry["env_var"].split(","):
                    for i in range(len(compile_cmd)):
                        compile_cmd[i] = re.sub(
                            "<" + env_var + ">",
                            get_env_var(env_var, debug_cmd=debug_cmd),
                            compile_cmd[i],
                        )
                    sim_cmd = re.sub(
                        "<" + env_var + ">",
...
def parse_iss_yaml(
    ...
    for entry in yaml_data:
        if entry["iss"] == iss:
            ...
            cmd = re.sub(
                r"\<tool_path\>",
                get_env_var(entry["tool_path"], debug_cmd=debug_cmd),
                cmd,
            )
            cmd = re.sub(
                r"\<tb_path\>", get_env_var(entry["tb_path"], debug_cmd=debug_cmd), cmd
            )
            ...
            if test["test"] == "riscv_csr_test":
                ...
                for i in range(0, batch_cnt):
                    ...
                    if lsf_cmd:
                        ...
                        logging.info(
                            "Running %s, batch %0d/%0d, test_cnt:%0d"
...
def gen(test_list, argv, output_dir, cwd):
    ...
    compile_cmd = []
    sim_cmd = ""
    compile_cmd, sim_cmd = get_generator_cmd(
        argv.simulator, argv.simulator_yaml, argv.cov, argv.exp, argv.debug
    )
    # Compile the instruction generator
    if not argv.so:
        do_compile(
...
def elf2bin(elf, binary, debug_cmd):
    ...
    cmd = "%s -O binary %s %s" % (
        get_env_var("RISCV_OBJCOPY", debug_cmd=debug_cmd),
        elf,
        binary,
    )
    ...
    for test in test_list:
        for i in range(0, test["iterations"]):
            ...
            cmd = (
             ...
             -T%s %s -o %s "
                % (
                    get_env_var("RISCV_CC", debug_cmd=debug_cmd),
                    asm,
                    cwd,
                    linker,
                    opts,
...
def tandem_postprocess(
    ...
    for n in range(nr_harts):
        logfile = log + ("_hart_%02g" % n)
        process_verilator_sim_log(
            logfile + ".log", logfile + ".csv", exit_on_ecall=exit_on_ecall
        )
    generate_yaml_report(
        tandem_report, target, isa, test_name, testlist, iss, False, iterations
    )
...
def analyze_tandem_report(yaml_path):
    ...
    try:
        ...
        logging.info(
            "TANDEM Result : %s (exit code %s) with %s mismatches and %s matches"
...
def generate_yaml_report(
    yaml_path, target, isa, test, testlist, iss, initial_creation, iteration=None
):
...
def run_test(
    ...
    for iss in iss_list:
        ...
        if log_format == 1:
            ...
            log = "%s/%s_sim/%s.%s" % (output_dir, iss, test_log_name, target)
        ...
        logging.info("[%0s] Running ISS simulation: %s ...done\n" % (iss, elf))
    ...
    if len(iss_list) == 2:
        compare_iss_log(
            iss_list, log_list, report, nr_harts=nr_harts, exit_on_ecall=exit_on_ecall
        )
        if compare_outputs:
            compare_output_logs(iss_list, log_list, report)
    if output_ref_file:
        compare_output_regexp(
            iss_list, log_list, os.path.abspath(output_ref_file), report
        )
        ...
        for test in test_list:
            if "no_iss" in test and test["no_iss"] == 1:
                ...
                for i in range(0, test["iterations"]):
                    ...
                    if iss == "ovpsim":
                        run_cmd(
                            cmd, timeout_s, check_return_code=False, debug_cmd=debug_cmd
                        )
                    else:
                        run_cmd(cmd, timeout_s, debug_cmd=debug_cmd)
                    logging.debug(cmd)
...
def iss_cmp(
    ...
    Args:
      ...
      exp            : Use experimental version
      debug_cmd      : Produce the debug cmd log without running
      compare_outputs: Compare outputs generated by the ISSs
      output_ref_file: Compare output of the ISS against a reference file
      exit_on_ecall  : Use ecalls as an exit marker
    """
    if debug_cmd:
        return
    ...
    for test in test_list:
        for i in range(0, test["iterations"]):
            ...
            logging.info(
                "Comparing ISS sim result %s/%s: %s" % (iss_list[0], iss_list[1], elf)
            )
            log_list = []
            run_cmd(("echo 'Test binary: %s' >> %s" % (elf, report)))
            for iss in iss_list:
                log_list.append(
                    "%s/%s_sim/%s_%d.%s" % (output_dir, iss, test["test"], i, target)
                )
            compare_iss_log(
                iss_list,
                log_list,
...
def compare_iss_log(
    ...
    if len(iss_list) != 2 or len(log_list) != 2:
        ...
        logging.info(
            "len(iss_list) = %s len(log_list) = %s" % (len(iss_list), len(log_list))
        )
        return
    ...
    for n in range(nr_harts):
        ...
        if n == 0 or "spike" not in iss_list:
            result = compare_trace_csv(
                csv_list[0], csv_list[1], iss_list[0], iss_list[1], report
            )
            logging.info(result)
        else:
            logging.info(
                "Skipping comparison for hart id > 0 (incompatibility with spike)\n"
            )
...
def compare_output_logs(iss_list, log_list, report):
    ...
    result = compare_text_traces(
        log_list[0] + ".out", log_list[1] + ".out", iss_list[0], iss_list[1], report
    )
    logging.info(result)
...
def save_regr_report(report):
    passed_cnt = run_cmd(r"grep -E '\[PASSED\]' %s | wc -l" % report).strip()
    failed_cnt = run_cmd(r"grep -E '\[FAILED\]' %s | wc -l" % report).strip()
    summary = "%s PASSED, %s FAILED" % (passed_cnt, failed_cnt)
    logging.info(summary)
    run_cmd(("echo -e '%s\\n\\n' >> %s" % (summary, report)))
    if failed_cnt != "0":
        failed_details = run_cmd(
            r"sed -e 's,.*_sim/,,' %s | grep -E '(csv|matched)' | uniq | sed -e "
            r"'N;s/\\n/ /g' | grep -E '\[FAILED\]'" % report
        ).strip()
        for line in failed_details.split("\n"):
            logging.info(line)
...
def read_seed(arg):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(
            "Bad seed ({}): must be a non-negative integer.".format(arg)
        )
...
def parse_args(cwd):
    ...
    parser.add_argument(
        "-o", "--output", type=str, help="Output directory name", dest="o"
    )
    parser.add_argument(
        "-tl",
        "--testlist",
    ...
    parser.add_argument(
        "--cmp_opts", type=str, default="", help="Compile options for the generator"
    )
    parser.add_argument(
        "--sim_opts", type=str, default="", help="Simulation options for the generator"
    )
    parser.add_argument("--gcc_opts", type=str, default="", help="GCC compile options")
    parser.add_argument(
        "--issrun_opts",
        type=str,
        default="+debug_disable=1",
        help="simulation run options",
    )
    parser.add_argument(
        "--isscomp_opts", type=str, default="", help="simulation comp options"
    )
    parser.add_argument(
        "--isspostrun_opts",
        type=str,
        default="0x0000000080000000",
    ...
    parser.add_argument("--isa", type=str, default="", help="RISC-V ISA subset")
    parser.add_argument(
        "--priv", type=str, default="msu", help="RISC-V ISA privilege models"
    )
    parser.add_argument(
        "-m",
        "--mabi",
        type=str,
        default="",
        help="mabi used for compilation",
        dest="mabi",
    )
    parser.add_argument(
        "--gen_timeout",
    ...
    parser.add_argument(
        "--iss_opts", type=str, default="", help="Any ISS command line arguments"
    )
    parser.add_argument(
        "--iss_timeout", type=int, default=900, help="ISS sim timeout limit in seconds"
    )
    parser.add_argument("--iss_yaml", type=str, default="", help="ISS setting YAML")
    parser.add_argument(
        "--simulator_yaml", type=str, default="", help="RTL simulator setting YAML"
    )
    parser.add_argument("--csr_yaml", type=str, default="", help="CSR description file")
    parser.add_argument(
        "-ct",
        "--custom_target",
        type=str,
    ...
    parser.add_argument(
        "--asm_tests", type=str, default="", help="Directed assembly tests"
    )
    parser.add_argument("--c_tests", type=str, default="", help="Directed c tests")
    parser.add_argument("--elf_tests", type=str, default="", help="Directed elf tests")
    parser.add_argument(
        "--log_suffix", type=str, default="", help="Simulation log name suffix"
    )
    parser.add_argument(
        "--exp",
        action="store_true",
    ...
    parser.add_argument(
        "-d", "--debug", type=str, default="", help="Generate debug command log file"
    )
    parser.add_argument(
        "--hwconfig_opts",
        type=str,
    ...
    parser.add_argument(
        "-l", "--linker", type=str, default="", help="Path for the link.ld"
    )
    parser.add_argument(
        "--axi_active",
        type=str,
    ...
    parser.add_argument(
        "--gen_sv_seed", type=int, default=0, help="Run test N times with random seed"
    )
    parser.add_argument(
        "--sv_seed",
        type=str,
        default=str(random.getrandbits(31)),
    ...
    parser.add_argument(
        "--nr_harts", type=int, default=1, help="Number of simulated harts"
    )
    parser.add_argument(
        "--compare_outputs",
        action="store_true",
        default=False,
        help="Also compares test stdout traces",
    )
    parser.add_argument(
        "--no_ecall_exit",
        action="store_true",
        default=False,
        help="Do not consider ecalls as exit markers",
    )
    parser.add_argument(
        "--output_ref_file",
        type=str,
    ...
    rsg.add_argument(
        ...
        type=read_seed,
        help=(
            "Randomization seed to use for first iteration of "
    ...
    rsg.add_argument(
        ...
        type=read_seed,
        help=(
            "Randomization seed to use for each test. "
    ...
    rsg.add_argument(
        ...
        type=str,
        help=(
            "Rerun the generator with the seed specification "
    ...
    if args.seed is not None:
        if args.iterations == 0:
            ...
            logging.error(
                "--seed is incompatible with setting --iterations greater than 1."
            )
            sys.exit(RET_FAIL)
...
def load_config(args, cwd):
    ...
    if not args.linker:
        my_link = Path(
            cwd + f"/../../config/gen_from_riscv_config/{args.target}/linker/link.ld"
        )
        if my_link.is_file():
            args.linker = (
                cwd
                + f"/../../config/gen_from_riscv_config/{args.target}/linker/link.ld"
            )
        else:
    ...
    if not args.custom_target:
        ...
        if args.target == "hwconfig":
            ...
            os.system(
                "cp ../../config/gen_from_riscv_config/%s/spike/spike.yaml ../../config/gen_from_riscv_config/hwconfig/spike/"
                % (base)
            )
            os.system(
                "cp ../../config/gen_from_riscv_config/%s/linker/*.ld ../../config/gen_from_riscv_config/hwconfig/linker/"
                % (base)
            )
        else:
            base = args.target
        ...
        if re.match(".*gcc_compile.*", args.steps) or re.match(
            ".*iss_sim.*", args.steps
        ):
            if (not args.mabi) or (not args.isa):
                sys.exit(
                    "mabi and isa must be specified for custom target %0s"
                    % args.custom_target
                )
        if not args.testlist:
    ...
    if (
        not "g" in args.isa
    ):  # LLVM complains if we add zicsr and zifencei when g is set.
        isa_extension_list.append("zicsr")
        isa_extension_list.append("zifencei")

    args.spike_params = (
        get_full_spike_param_args(args.spike_params) if args.spike_params else ""
    )
...
def incorrect_version_exit(tool, tool_version, required_version):
    if tool == "Spike":
        logging.error(
            f"Please clean up Spike by executing: rm -r tools/spike verif/core-v-verif/vendor/riscv/riscv-isa-sim/build"
        )
    logging.error(
        f"You are currently using version {tool_version} of {tool}, should be: {required_version}. Please install or reinstall it with the installation script."
    )
    sys.exit(RET_FAIL)
...
def check_spike_version():
    ...
    spike_hash = subprocess.run(
        "git log -1 --pretty=tformat:%h",
        capture_output=True,
        text=True,
        shell=True,
        cwd=os.environ.get("SPIKE_SRC_DIR"),
    )
    spike_version = "1.1.1-dev " + spike_hash.stdout.strip()

    # Get Spike User version
    get_env_var("SPIKE_PATH")
    user_spike_version = subprocess.run(
        "$SPIKE_PATH/spike -v", capture_output=True, text=True, shell=True
    )
...
def check_verilator_version():
    ...
    if REQUIRED_VERILATOR_VERSION != verilator_version:
        incorrect_version_exit(
            "Verilator", verilator_version, REQUIRED_VERILATOR_VERSION
        )
...
def openhw_process_regression_list(
    testlist, test, iterations, matched_list, riscv_dv_root
):
...
def main():
    ...
    try:
        ...
        if args.axi_active == "yes":
            args.issrun_opts = (
                args.issrun_opts + " +uvm_set_config_int=*uvm_test_top,force_axi_mode,1"
            )
        elif args.axi_active == "no":
            args.issrun_opts = (
                args.issrun_opts + " +uvm_set_config_int=uvm_test_top,force_axi_mode,0"
            )

        if args.gen_sv_seed > 0 and args.sv_seed != "1":
            logging.error(
                "You cannot use gen_sv_seed and sv_seed options at the same time"
            )
        ...
        if args.iss:
            ...
            if "spike" in args_list:
                ...
                args_list.insert(
                    0, "spike"
                )  # Insert 'spike' at the beginning of the list
        ...
        if args.verilog_style_check:
            ...
            if style_err:
                logging.info("Found style error: \nERROR: " + style_err)
        ...
        for i in range(args.gen_sv_seed):
            ...
            if test_executed == 0:
                if not args.co:
                    openhw_process_regression_list(
                        args.testlist, args.test, args.iterations, matched_list, cwd
                    )
                    logging.info(
                        "CVA6 Configuration is %s and target is %s"
                        % (args.hwconfig_opts, args.target)
                    )
                        ...
                        if yaml_needs:
                            ...
                            for keys in needs.keys():
                                if args.hwconfig_opts[keys] != needs[keys]:
                                    logging.info(
                                        "Removing test %s CVA6 configuration can not run it"
                                        % entry["test"]
                                    )
                    ...
                    if (
                        len(matched_list) == 0
                        and len(asm_directed_list) == 0
                        and len(c_directed_list) == 0
                    ):
                        sys.exit("Cannot find %s in %s" % (args.test, args.testlist))

                    for t in c_directed_list:
                        copy = (
                            re.sub(r"(.*)\/(.*).c$", r"cp \1/\2.c \1/", t["c_tests"])
                            + t["test"]
                            + ".c"
                        )
                        run_cmd("%s" % copy)
                        t["c_tests"] = (
                            re.sub(r"(.*)\/(.*).c$", r"\1/", t["c_tests"])
                            + t["test"]
                            + ".c"
                        )
